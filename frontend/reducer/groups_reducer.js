...
const GroupsReducer = (oldState={},action) => {
  ...
  switch (action.type) {
    ...
    case RECEIVE_GROUP:
      let group = [];
      if (typeof action.members === "undefined"){
        group = merge(action.group,{members: [], events: action.events});
      } else {
        group = merge(action.group,{members: action.members, events: action.events});
      }
